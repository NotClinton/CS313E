    ...
    def create_adjacency_matrix(self):
        ...
        for vertex in self.vertices:
            for edge in vertex.edges:
                adj_matrix[vertex.index][edge] = 1
    ...
    def bfs(self, start_index, color):
        ...
        while not queue.is_empty():
            ...
            if curvertex.color == self.vertices[start_index].color:
                curvertex.visit_and_set_color(color)

            for neighborindex in curvertex.edges:
                neighbor = self.vertices[neighborindex]
...
def create_graph(data):
    ...
    verticeslist = data_[2:2+numvertices]
    for i, string in enumerate(verticeslist):
        vertices = string.split(",")
        x = int(vertices[0])
        y = int(vertices[1])
        color = vertices[2]
        graph.vertices += [ColoredVertex(i, x, y, color)]
    # create edges between vertices - edge info has the format "from_index,to_index"
    edgeslist = data_[3+numvertices:3+numvertices+numedges]
    for string in edgeslist:
        edges = string.split(",")
        from_index = int(edges[0])
        to_index = int(edges[1])
...
def main():
    ...
    for row in graph.create_adjacency_matrix():
        print(row)
